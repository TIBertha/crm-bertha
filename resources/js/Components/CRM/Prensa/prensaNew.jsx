...
import {ajaxPrensaGetData, ajaxPrensaNew} from "../../Functions/Prensa.jsx"
...
class PrensaNew extends Component {
    constructor(props) {
        super(props);
        this.state = {
            url: props.url,
            tags: [{ id: 'bertha', text: 'bertha' }, { id: 'prensa', text: 'prensa' }],
            fecha: new Date(),
            fuente: '',
            titulo: '',
            contenido: '',
    ...
    handleSingularUpload(files, type){
        ...
        data = Object.assign({}, this.state.imagen);
    ...
    processSingularUpload(uploadedFile, data, type){
        ...
        axios.post("/ajax-prensa-adjunto", datos, {})
            .then(res => {
    ...
    handleChange(e, tipo = '', campo = '') {
        ...
        if(tipo == 'retrato'){
            ...
            imageCompression((e.target.files[0]), options).then(function (compressedFile) {
    ...
    save(e){
        ...
        ajaxPrensaNew(this.state).then(r => {
    ...
    render() {
        ...
        return(
            <>
                <TitleLabel content={'Agregar nuevo artículo de prensa'} />
...
export default withRouter(PrensaNew);
